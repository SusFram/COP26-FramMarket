...
Actions that were most strongly supported (> 80% of 'yes' responses) are shown in Table \@ref(tab:yesTab).

```{r yesTab}
t <- yes_t[yes_response == "Yes" & pc_yes > 80][order(-pc_yes)]

makeFlexTable(head(t[, .(Question = text, `% yes` = pc_yes)]))
```

Actions that were least strongly supported (> 10% of 'no' responses) are shown in Table \@ref(tab:noTab).

```{r noTab}
t <- no_t[no_response == "No" & pc_no > 10][order(-pc_no)]

makeFlexTable(head(t[, .(Question = text, `% no` = pc_no)]))
```
